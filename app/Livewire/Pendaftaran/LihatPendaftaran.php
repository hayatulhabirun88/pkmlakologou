...
class LihatPendaftaran extends Component
{
    use WithPagination;
    ...
    public function deleteshow($id, $nama_pasien)
    {
    ...
    {
        $loketlayanan = LoketLayanan::findOrFail($id);
        $loketlayanan->delete();
        session()->flash('deletesuccess', 'Data Pendaftaran pasien berhasil dihapus!');
        $this->isOpenDelete = false;
    }
    public function render()
    {
        $loketlayanan = LoketLayanan::latest()->paginate(10);
        return view('livewire.pendaftaran.lihat-pendaftaran', compact(['loketlayanan']));
    }
}
